...
::       accented characters (é, à, ü…) in folder names more reliably.
...
:: Convert current Windows directory to WSL path (e.g. C:\Users\foo → /mnt/c/Users/foo)
for /f "delims=" %%P in ('wsl -- wslpath -u "%WIN_CWD%"') do set "WSL_CWD=%%P"

:: Resolve WSL home directory
for /f "delims=" %%H in ('wsl -- bash -c "echo $HOME"') do set "WSL_HOME=%%H"
...
pushd %TEMP%
wsl "%WSL_PYTHON%" "%WSL_SCRIPT%" --workdir "%WSL_CWD%"
popd
